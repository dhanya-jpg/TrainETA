...
export const LiveTrainMap: React.FC<LiveTrainMapProps> = ({ train, onSelectStation }) => {
  ...
  // Calculate current bearing angle
  ...
  useEffect(() => {
    ...
    // Choose high performance crisp CartoDB tiles
  ...
  // Update Tile Layer if dark mode class changes
  useEffect(() => {
    ...
    train.stops.forEach((stop, index) => {
      ...
      const tooltipContent = `
        <div style="font-family: inherit; font-size: 12px; line-height: 1.4; padding: 2px;">
          <div style="font-weight: 700; color: #3B82F6; font-family: monospace;">${stop.stationCode}</div>
          <div style="font-weight: 600; color: #F1F5F9;">${stop.stationName}</div>
          <div style="color: #94A3B8; font-size: 11px; margin-top: 2px;">
            Arr: ${stop.scheduledArrival} • Dep: ${stop.scheduledDeparture}
          </div>
          ${stop.actualArrival ? `<div style="color: #34D399; font-size: 11px; font-weight: 600;">Status: ${stop.status} (+${stop.delayMinutes}m)</div>` : ''}
        </div>
      `;
      ...
      marker.on('click', () => {
        if (onSelectStation) {
          onSelectStation(stop.stationCode);
        }
      });
  ...
  useEffect(() => {
    ...
    const trainHtml = `
      <div style="position: relative; display: flex; align-items: center; justify-content: center; pointer-events: auto; user-select: none;">
        ...
        <!-- Rotating Train Engine Indicator -->
        <div style="
          position: relative;
          width: 38px;
          height: 38px;
        ...
        <!-- Train Speed & Number Floating Pill -->
        <div style="
          position: absolute;
          top: -26px;
          white-space: nowrap;
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (!userAccuracyCircleRef.current) {
          userAccuracyCircleRef.current = L.circle([loc.lat, loc.lng], {
            radius: Math.min(500, loc.accuracy),
            color: '#3B82F6',
            fillColor: '#3B82F6',
            fillOpacity: 0.1,
            weight: 1,
          ...
          userAccuracyCircleRef.current.setLatLng([loc.lat, loc.lng]);
          userAccuracyCircleRef.current.setRadius(Math.min(500, loc.accuracy));
        }
      },
      () => { setIsLocating(false); },
  ...
  return (
    <div className="relative isolate w-full h-full min-h-[360px] sm:min-h-[460px] bg-surface rounded-2xl sm:rounded-3xl overflow-hidden border border-border shadow-xs flex flex-col text-ink">
      ...
      <div className="absolute top-2.5 left-2.5 right-2.5 sm:top-3 sm:left-3 sm:right-3 z-1000 flex items-start justify-between gap-2 pointer-events-none">
        ...
        <div className="flex flex-col gap-1.5 pointer-events-auto shrink-0 z-1000">
          <button
            type="button"
